#ifndef ROBOTCGMRES_MULTIPLE_SHOOTING_OCP_HPP_
#define ROBOTCGMRES_MULTIPLE_SHOOTING_OCP_HPP_
...
public:
  ...
  //   cost_function: Pointer of the cost function.
  //   constraints: Pointer of the cost function.
  //   N: The division number of the horizon.
  MultipleShootingOCP(const Robot* robot_ptr,  
                      const CostFunctionInterface* cost_function,
                      const ConstraintsInterface* constraints, const int N);
  ...
  MultipleShootingOCP(const ZeroHorizonOCP&) = delete;

  // Prohibits copy operator.
  MultipleShootingOCP& operator=(const ZeroHorizonOCP&) = delete;
...
private:
  ...
  //   v: Generalized velocity. The size must be dimv.
  //   a: Generalized acceleration. The size must be dimv.
  //   fext: Contact forces. The size must be dimv.
  //   mul_passive: The Lagrange multiplier with respect to the passive joint 
  //      constraints. The size must be dim_passive.
  //   mul_constriants: The Lagrange multiplier with respect to the user-defined 
  //      equality constraints. The size must be dim_constraints.
  ...
  inline void computeOptimalityResidualWithContacts(
      const double t, const double* q, const double* v, const double* a, 
      const double* fext, const double* mul_contact, const double* mul_passive, 
...
#endif // ROBOTCGMRES_MULTIPLE_SHOOTING_OCP_HPP_