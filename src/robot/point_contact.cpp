...
void PointContact::loadPinocchioModel(pinocchio::Model& model) {
    parent_joint_id_ = model.frames[contact_frame_id_].parent;
    dimv_ = model.nv;
    jXf_ = model.frames[contact_frame_id_].placement;
    fXj_ = jXf_.inverse().toActionMatrix().transpose();
    J_local_ = Eigen::MatrixXd::Zero(6, dimv_);
    J_frame_ = Eigen::MatrixXd::Zero(6, dimv_);
    joint_v_partial_dq_ = Eigen::MatrixXd::Zero(6, dimv_);
    joint_a_partial_dq_ = Eigen::MatrixXd::Zero(6, dimv_);
    joint_a_partial_dv_ = Eigen::MatrixXd::Zero(6, dimv_);
    joint_a_partial_da_ = Eigen::MatrixXd::Zero(6, dimv_);
}
...
void PointContact::resetContactPointByCurrentState(pinocchio::Data& data) {
...
void PointContact::contactJacobian(const pinocchio::Model& model, 
  ...
  pinocchio::getFrameJacobian(model, data, contact_frame_id_, 
                              pinocchio::LOCAL, 
                              J_contacts.block(block_start_row, 
                                               block_start_column, 6, dimv_));
}
...
void PointContact::baumgarteDerivatives(const pinocchio::Model& model, 
  ...
  pinocchio::getFrameJacobian(model, data, contact_frame_id_, 
                              pinocchio::LOCAL_WORLD_ALIGNED, J_frame_);
  baumgarte_partial_dq.template block(block_start_row, block_start_column, 3, 
                                      dimv_) 
      = joint_a_partial_dq_.template topRows<3>()
          + baumgarte_alpha_ * joint_v_partial_dq_.template topRows<3>()
          + baumgarte_beta_ * J_frame_.template topRows<3>();
  baumgarte_partial_dv.template block(block_start_row, block_start_column, 3,
                                      dimv_) 
      = joint_a_partial_dv_.template topRows<3>()
          + baumgarte_alpha_ * joint_a_partial_da_.template topRows<3>();
  baumgarte_partial_da.template block(block_start_row, block_start_column, 3,
                                      dimv_) 
      = joint_a_partial_da_.template topRows<3>();
}
